...
{
    return angleToHeading(angleOfLine(a));
}
...
attMapper.getGeoCoord(start, startG);
attMapper.getGeoCoord(end, endG);
std::priority_queue<StreetNode*> queueLine;
MyMap<GeoSegment,StreetNode*> closed;
int times = 0;
vector<StreetSegment>jjj(segMapper.getSegments(startG));





head = new StreetNode(nullptr, jjj[0], jjj[0].segment.start, endG, startG);
bool isFound =false;
queueLine.push(head);
...
string before ="";
double km = 0;
vector<StreetSegment> jo;
while (goal != nullptr)
{
    before = goal->s.streetName;
     jo.push_back(goal->s);
     goal = goal->parent;
     if(goal == nullptr)
     break;
     if(goal != nullptr && goal->s.streetName != before)
     {
     dirs++;
     cout<<goal->s.streetName<<endl;
     }
     before = goal->s.streetName;
    jo.push_back(goal->s);
    km += goal->length;
